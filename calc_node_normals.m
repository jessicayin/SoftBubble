function [normal, M, Fn] = calc_node_normals(node_xyz, element_node)
...
%
    ...
    t3_W(1:3,1:3) = node_xyz(1:3,element_node(1:3,element));
    ...
    % Orientation of the triangle frame T in the world frame W.
    R_WT = [u1 u2 u3];

    % Pose of frame T in the world frame W. To = p1.
    X_WT = [R_WT p1;
           zeros(1, 3) 1];
    ...
    % Triangle nodes measured and expressed in T.
    res = X_TW * [t3_W; 1 1 1];
    t3_T = res(1:3, :);
    
    % Gauss points measured and expressed in the triangle frame T.
    phys_xy_T(1:3,1:quad_num) = reference_to_physical_t3(t3_T, quad_num, quad_xy);    
    
%
%  Map the quadrature points QUAD_XY to points PHYS_XY in the physical element.
%
%    phys_xy_W(1:3,1:quad_num) = reference_to_physical_t3( t3_W, quad_num, quad_xy );
    
    % Gauss points measured and expressed in the triangle frame T.
    %res = X_TW * [phys_xy_W; 1 1 1];
    %phys_xy_T = res(1:3, :);
...
%
      for test = 1 : 3
        ...
        f(i,1) = f(i,1) + w(quad,1) * normal_W(1) * bi;
        f(i,2) = f(i,2) + w(quad,1) * normal_W(2) * bi;
        f(i,3) = f(i,3) + w(quad,1) * normal_W(3) * bi;
%
...
%
  node_bc = dirichlet_condition ( node_num, node_xyz );
...
function node_r = residual_poisson ( node_num, node_xyz, node_condition, ...
  element_num, element_node, quad_num, a, f, node_u )
...
%  Make a copy of the element.
%
    t3(1:2,1:3) = node_xyz(1:2,element_node(1:3,element));
%
%  Map the quadrature points QUAD_XY to points PHYS_XY in the physical element.
%
